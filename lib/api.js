...
module.exports = {
	...
	getTableOfContents(params, text) {
		...
		// exclude dupe chapter titles (ids)
		var id_map = {};
		chapters.forEach( function(item) {
			id_map[item.id] = (id_map[item.id] || 0) + 1;
		} );
		
		var toc = '';
		chapters.forEach( function(item) {
			if (id_map[item.id] > 1) return;
			var tabs = '';
			...
			async.eachSeries( files,
				...
				function(err) {
					...
					matches.forEach( function(match) {
						...
						if (match.code) match.line = '<code>' + match.line.replace(/\`/g, '').replace(/\\/g, '') + '</code>';
